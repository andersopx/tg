...
def test_price_to_beat_fallback_enabled_by_default():
    s = Path('config.py').read_text()
    assert 'PRICE_TO_BEAT_FALLBACK_ENABLED' in s
    assert '_env_bool("PRICE_TO_BEAT_FALLBACK_ENABLED", True)' in s


def test_trader_does_not_hard_skip_missing_price_to_beat_when_fallback_available():
    ...
    assert 'Orderbook/edge/slippage checks still apply' in s
